...
const mapToRemote = (
	...
	return {
		...
		location: options.has_ended ? useLocation.getState().location : undefined,
	};
};
...
const playerIndexesToIds = (state: GameState, indexes: number[]): number[] => {
	return indexes
		.filter(
			(index) => state.players[index] && state.players[index].id !== undefined,
		)
...
const playerIdsToIndexes = (
	...
	return ids.map((id) => playerIds.indexOf(id)).filter((index) => index !== -1);
};
