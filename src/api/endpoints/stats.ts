...
export async function getRankedCards(): Promise<IRankedCardResponse[]> {
    const response = await client.get<IRankedCardResponse[]>("/api/ranked_cards/");
...
export async function getUserStats(userId: number): Promise<IUserStatsResponse> {
    const response = await client.get<IUserStatsResponse>(`/api/stats/${userId}/`);
    return response.data;
}
